...
function finalCompareGo() {
  ...
  if (theClient.sum > 21) {
    // youLose.play();
    $("#player-result-big-answer").text("USTED PIERDE");
    ...
    // youWin.play();
    $("#player-result-big-answer").text("GANASTE");
    ...
    $("#player-result-big-answer").text("TIRAR");
    // $("#player-result-big-sum").text(theClient.bet)
    // $("#player-result-big-plus-minus").text("+")
  } else {
    // youLose.play();
    $("#player-result-big-answer").text("GANA CRUPIER");
    $("#player-result-big-sum").text(theClient.bet);
    $("#player-result-big-plus-minus").text("-");
    $("#player-result-sum-box").addClass("color-red");
  }
        ...
        if (dealer.sum > 21) {
          ...
          for (let x = 0; x < playerSlotHTML.length; x++) {
            if (players[i].clientId === playerSlotHTML[x]) {
              ...
              $(".player-result:eq(" + x + ")").text("PIERDES");
            }
          }
        }
    ...
    for (let i = 0; i < players.length; i++) {
      if (players[i].hasLeft === true) {
        if (players[i].clientId === playerSlotHTML[x]) {
          ...
          <div><button class="ready hide-element">HACER APUESTA</button></div>
...
function updateMediaQuery() {
  if ($(".players").css("width") === "90px") {
    ...
    $("#invite-label").text("Coloca el cursor para ver el enlace de invitación");
