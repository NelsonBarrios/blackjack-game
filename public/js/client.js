...
ws.onmessage = (message) => {
  ...
  if (response.method === "create") {
    ...
    if (offline === true) {
      ...
      $("#users-online-label").text("MODO OFFLINE");
    }
  }
  ...
  if (response.method === "joinClient") {
    ...
    for (let i = 0; i < spectators.length; i++) {
      if (spectators[i].nickname === "") spectators[i].nickname = "jugador";
      ...
      <li class="users-list-box">
        <div class="users-list-info">
          ...
          `</div>
          <div>Saldo: <span class="users-list-balance">` +
    ...
    for (let x = 0; x < players.length; x++) {
      if (players[x].clientId === playerSlotHTML[i]) {
        ...
        if (players[x].sum > 21) {
          ...
          $(".player-result:eq(" + z + ")").text("PIERDE");
        }
      }
    }
  }
